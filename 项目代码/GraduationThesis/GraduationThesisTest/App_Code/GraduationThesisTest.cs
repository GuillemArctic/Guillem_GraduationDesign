...
public class GraduationThesisTest : System.Web.Services.WebService
{
    ...
    /// <returns></returns>
    [WebMethod(Description = "添加用户基本信息同时完成注册")]
    ...
    /// <param name="UserRegiste_Phone"></param>
    /// <returns></returns>
    [WebMethod(Description ="查询人员基本信息")]
    public string Select_User_RegisteInfo(string UserRegiste_Phone)
    {
        try
        {
            List<User_RegisteInfo> user_RegisteInfos = new List<User_RegisteInfo>();
            User_RegisteInfo user_RegisteInfo = new User_RegisteInfo();
            using (SqlConnection con = new SqlConnection(strcon))
            {
                con.Open();
                SqlCommand command = new SqlCommand("select * from User_RegisteInfo where UserRegiste_Phone = '" + UserRegiste_Phone + "'", con);
                SqlDataReader dr = command.ExecuteReader();
                if (dr.Read())
                {
                    user_RegisteInfo.UserRegiste_ID = int.Parse(dr[0].ToString());
                    user_RegisteInfo.UserLogin_ID = dr[1].ToString();
                    user_RegisteInfo.UserRegiste_Name = dr[2].ToString();
                    user_RegisteInfo.UserRegiste_Sex = dr[3].ToString();
                    user_RegisteInfo.UserRegiste_Phone = dr[4].ToString();
                    user_RegisteInfo.UserRegiste_Cardid = dr[5].ToString();
                    user_RegisteInfo.UserRegiste_Email = dr[6].ToString();
                    user_RegisteInfos.Add(user_RegisteInfo);
                }
                else
                    return null;
                con.Close();
            }
            return JsonConvert.SerializeObject(user_RegisteInfos);
        }
        catch (Exception ex)
        {
            return ex.ToString();
    ...
    {
        public int UserRegiste_ID { get; set; }
